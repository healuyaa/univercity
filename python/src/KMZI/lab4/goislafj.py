...
class Quest1:
    ...
    def add_points(self, P, Q):
        ...
        if P != Q:
            ...
            m = ((3 * 2 ** x1 + self.a) * pow(2 * y1, -1, self.p))
    ...
    def multiply_point(self, k, P):
        ...
        for i in range(k.bit_length()):
            ...
            P = self.add_points(P, Q)
        return Q
    
    def encrypt(self, plaintext, public_key):
        k = self.gen_secret_key()
        C1 = self.multiply_point(k, self.G)
        C2 = []
        for char in plaintext:
            x, y = self.multiply_point(k, public_key[char])
            C2.append((x, y))
        return C1, C2
    ...
    def decrypt(self, C1, C2, private_key):
        ...
        for x, y in C2:
            decrypt_char = None
            for char, point in self.alphabet.items():
                if point == (x, y):
                    decrypt_char = char
                    break
            if decrypt_char is not None:
...
def main():
    ...
    plaintext = "hello"
    for char in plaintext:
        public_key[char] = obj.multiply_point(obj.gen_secret_key(), obj.G)
    C1, C2 = obj.encrypt(plaintext, public_key)
    decrypted_text = obj.decrypt(C1, C2, obj.gen_secret_key())
